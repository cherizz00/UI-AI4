                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    {testimonials.map((test, i) => (
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className="glass-card p-6 md:p-10 rounded-2xl md:rounded-[40px] relative overflow-hidden group min-h-[140px] md:min-h-auto flex flex-col justify-between"
                        >
                            <Quote className="absolute top-4 right-4 md:top-8 md:right-8 text-brand-orange/10 group-hover:text-brand-orange/20 transition-colors md:w-6 md:h-6" size={16} />

                            <p className="text-slate-800 text-base md:text-lg font-semibold mb-4 md:mb-10 relative z-10 leading-snug tracking-tight text-left italic">
                                "{test.quote}"
                            </p>

                            <div className="relative z-10 text-left">
                                <h4 className="text-slate-900 font-serif text-lg md:text-xl mb-0.5 md:mb-1 tracking-tight leading-none">{test.author}</h4>
                                <p className="text-brand-orange text-[10px] md:text-[11px] font-bold uppercase tracking-wider leading-none">{test.firm}</p>
                            </div>
